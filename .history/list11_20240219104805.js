...
function drawCharts(Objects, selectedItems) {
  ...
  selectedItems.map((el) => {
    console.log("chartsDataArrays[el] is ", chartsDataArrays[el]);
    selectedItemsObjects.push({
      x: chartsDataArrays.date,
      y: chartsDataArrays[el] ? chartsDataArrays[el] : 0,
      name: arabicTranslation[el],
      type: selectedType,
      hovertemplate: `%{x} :${arabicTranslation.date}<br>%{y} :${arabicTranslation[el]}<br>`,
    });
  });
  console.log("chartsDataArrays: ", chartsDataArrays);
  let data1 = [];
  if (
...
function updateCharts(chartsData) {
  ...
  if (
    ...
    console.log("securitiesFilter are ", securitiesFilter);
    ...
    console.log("sectorsFilter are ", sectorsFilter);
