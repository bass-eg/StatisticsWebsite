...
function prepareDataForCharts(Objects) {
  ...
  Objects.map((el) => {
    el.map((inner) => {
      ...
      const filteredDetails = filterDetailsByDateRange(
        inner.details,
        startDate,
        endDate
      );

      console.log("filteredDetails are ", filteredDetails);
      for (let j = 0; j < filteredDetails.length; j++) {
        for (let key in chartsDataArrays) {
          chartsDataArrays[key].push(filteredDetails[j][key]);
        }
      }
    });
  });
...
const filterDetailsByDateRange = (details, startDate, endDate) => {
  return details
    .filter((a) => {
