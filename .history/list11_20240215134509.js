...
function updateCharts(chartsData) {
  ...
  let selectedSectorsCompanyObjects = [];
  ...
  if (
    ...
    if (securitiesFilter) {
      selectedSectorsCompanyObjects.push(...securitiesFilter);
    }
    ...
    if (sectorsFilter) {
      selectedSectorsCompanyObjects.push(...sectorsFilter);
    }

    let selectChartItemsValue = $("#selectChartItems").val();
    if (
      selectedSectorsCompanyObjects.length === 0 ||
      ...
      drawCharts(selectedSectorsCompanyObjects, selectChartItemsValue);
    }
  }
}
